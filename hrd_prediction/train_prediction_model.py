...
def main(args):
    ...
    for extraction_model in ["UNI", "UNI_2", "RetCCL", "GPFM", "CONCH", "Virchow_2"]:
    
        patient_data.reset_index(drop=True, inplace=True)
        
        for seed in [2, 42, 55, 878, 10101]:
            if args.cohort in ["UCEC-LUAD", "LUAD-UCEC"]:
                train_cohort, test_cohort = args.cohort.split("-")
                # train_feature_path = f"{args.dataset_path}/TCGA/{train_cohort}/features/{args.extraction_model}"
                # test_feature_path = f"{args.dataset_path}/CPTAC/{test_cohort}/features/{args.extraction_model}"
            ...
            print("NOW TRAINING WITH: " + extraction_model)
            train_CAMIL_model(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cohort", type=str, required=True, 
                        choices=["UCEC", "LUAD", "UCEC-LUAD", "LUAD-UCEC"], help="Cohort to filter the data.") #TODO add more cohorts
