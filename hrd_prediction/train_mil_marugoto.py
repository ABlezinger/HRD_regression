...
from marugoto_helpers._mil import train_marugoto, train_marugoto_crossval, deploy
...
def train_CAMIL_model_crossval(
    ...
    output_path = Path(f"hrd_prediction/models/CAMIL_crossval/{extraction_model}_{cohort}")
    os.makedirs(output_path, exist_ok=True)
    info = {
    ...
    df = get_patient_df(data_df, data_path=feature_path, feature_extractor=extraction_model) #categories
    ...
    info['folds'] = [
        {
            part: list(df.patient_id[folds[fold_i][i]])
    ...
    for fold, (train_idxs, test_idxs) in enumerate(folds):
        ...
        if (preds_csv := fold_path/'patient-preds.csv').exists():
            ...
            learn = train_marugoto_crossval(
                ...
                n_epochs=epochs) #added weights #fold_weights_train=fold_weights_train
            learn.export()
